// Tests for the gRPC translation server using mock translator and cache so we don't need Ollama or Redis.
// These tests run on every deploy (see Dockerfile) to verify the whole implementation.
package grpc
...
// fakeTranslator returns a fixed string and records whether Translate was called.
...
// fakeCache implements Cache for tests; Get returns the value set by the test.
